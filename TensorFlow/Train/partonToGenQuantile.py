...
@click.option('--networkName', default="partonToGenFull", help='Name of network')
def main(hidden, width, alpha, initiallr, batch, cycles, epochs, patience, dataname, networkname):
    ...
    data = np.load("validationData.npy").T
    partonData = data[0:4,:]
    genData = data[4:8,:]
    recoData = data[8:12,:]
    genData[0,:] = np.log(genData[0,:])
    ...
    normInfoOut=[[0,1],[0,1],[0,1],[0,1]]
    print(trainIn.shape, valIn.shape, valOut.shape, valOut.shape)
