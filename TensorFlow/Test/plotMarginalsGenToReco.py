...
extraLabelTerms = ["pt", "eta", "phi", "m"]
for j in range(2,4):
    for i in range(len(discretizedRanges[j])-1):
        allCounts=[]
        allEdges=[]
        quantCounts=[]
        quantEdges=[]
        ...
        true=[]
        cutMin = discretizedRanges[j][i]
        cutMax = discretizedRanges[j][i+1]
        extraLabel = extraLabelTerms[j]+"_"+str(cutMin)+"_"+str(cutMax)+"_"
        totalCounts = 0
        for x in range(totalSamples//batchSize+1):
            ...
            weights = np.where(genData[:,j]>cutMin, weights, weights*0)
            weights = np.where(genData[:,j]<cutMax, weights, weights*0)
