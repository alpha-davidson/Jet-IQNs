...
def main(traineataname, testdataname, networkname, h5):
    ...
    for x in range(totalSamples//batchSize+1):
        ...
        for k in range(4):
            weights = np.where(genData[:,k]>cut[k][0], weights, weights*0)
            weights = np.where(genData[:,k]<cut[k][1], weights, weights*0)
    ...
    for x in range(4):      
        ...
        plt.tight_layout()
        fig.savefig(extraLabel+"_"+titles[x].replace(" ","_")+"_g2r.pdf")
        print("saving")
        fig.subplots_adjust(wspace=0.0, hspace=0.1)
        fig.show()
