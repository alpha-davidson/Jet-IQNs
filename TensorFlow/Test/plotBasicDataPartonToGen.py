...
for x in range(totalSamples//batchSize+1):
    ...
    for y in range(4):
        partonCounts, edges = np.histogram(partonData[:,y], range=ranges[y], bins=bins, weights=weights)
        ...
        qEdges = qEdges[1:]
        if(x==0):
            allCounts.append([partonCounts, predCounts[:,y,:], genCounts])
            allEdges.append(edges)
            quantCounts.append([qCounts])
            quantEdges.append(qEdges)
        else:
            allCounts[y][0]+=partonCounts
...
for x in range(4):
    ...
    fig.savefig("calibration_"+titles[x].replace(" ","_")+"p2g.png")
    fig.show()
...
for x in range(4):      
    ...
    ax1.scatter(allEdges[x], allCounts[x][2], label="gen",  color="k",facecolors='none', marker="o", s=5, linewidth=0.5)
    ...
    plt.tight_layout()
    fig.savefig("mD_"+titles[x].replace(" ","_")+"p2g.png")
    fig.subplots_adjust(wspace=0.0, hspace=0.1)
    fig.show()
