...
class QuantileNet(tf.keras.Sequential):
    """
    ...
    def __init__(self, grad_loss_scale=100, tanh_loss_scale=100,
                 network_type="normalizing", clip=1e-7, name="QN"):
        """
        ----------
    ...
    def normalizing_call(self, inputs):
        ...
        quantiles = tf.random.uniform(shape=[1, count], minval=-3, maxval=3,
                                      dtype=tf.float32)
    ...
    def no_normalizing_call(self, inputs):
        ...
        ## Find number of example
        for input_val in tf.transpose(inputs):
            count += 1
        grad_loss = np.float32(0.0)

    
        quantiles = tf.random.uniform(shape=[1,count], minval=-3, maxval=3,dtype=tf.float32) 
...
def sample_net(quantile_object, quantile_samples, inputs, input_count,
    ...
    def predict_normalizing(val):
        ...
        # make predictions
        for x in range(len(quantile_object.layers)):
            layer = quantile_object.layers[x]
            val = layer(val)
        ...
        #val = tf.clip_by_value(val, -clip, clip)
