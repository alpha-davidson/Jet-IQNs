...
def main():
    ...
    # training data, 1 dimensional input, uniform noise with sd 0.1*x
    x_vals_t = np.array([np.linspace(-3, 3, samples)]).T
    noise_t = np.random.uniform(-1, 1, size=x_vals_t.shape)
    noise_t = noise_t*np.sqrt(3)*0.1*x_vals_t
    ...
    noise_v = np.random.uniform(-1, 1, size=x_vals_v.shape)
    noise_v = np.sqrt(3)*0.1*x_vals_v*noise_v
    y_vals_v = func(x_vals_v)+noise_v
    ...
    for x in range(4):
        ...
        # Save the network
        network.save("Trig_Uniform", save_traces=False)
